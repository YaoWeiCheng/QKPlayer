//
//  QKCustomLandScapeControlView.swift
//  QKPlayer
//
...
class QKCustomLandScapeControlView: UIView {
    ...
    override func layoutSubviews() {
        ...
        min_y = 0
        min_w = min_view_w
        min_h = QKPlayer_IsIphoneX ? min_view_h : 80
        self.topToopView.frame = CGRect(x: min_x, y: min_y, width: min_w, height: min_h)
        let status = UIApplication.shared.statusBarOrientation == UIInterfaceOrientation.landscapeLeft || UIApplication.shared.statusBarOrientation == UIInterfaceOrientation.landscapeRight
        min_x = QKPlayer_IsIphoneX && status ? 44 : 15
        if #available(iOS 13, *) {
            min_y = status ? 10 : QKPlayer_IsIphoneX ? 40 : 20
        } else {
            min_y = QKPlayer_IsIphoneX && status ? 10 : QKPlayer_IsIphoneX ? 40 : 20
        }
        min_w = 40
        min_h = 40
        self.backBtn.frame = CGRect(x: min_x, y: min_y, width: min_w, height: min_h)
        ...
        min_x = 0
        min_h = QKPlayer_IsIphoneX ? 100 : 73
        ...
        min_h = 20
        min_y = (self.bottomToolView.frame.height - min_h) / 2
        min_x = min_view_w - min_w - min_margin * 2
        ...
        if !self.isShow {
            self.topToopView.frame.origin.y = -self.topToopView.frame.height
            self.bottomToolView.frame.origin.y = self.frame.height
            self.playOrPauseBtn.alpha = 0
        } else {
            self.topToopView.frame.origin.y = 0
    ...
    func setupUI() {
        ...
        self.topToopView.addSubview(self.backBtn)
        self.backBtn.setImage(QKUtils.imageName(name: "QKPlayer_back_full"), for: .normal)
        let topImage = QKUtils.imageName(name: "QKPlayer_bottom_shadow")
        bottomToolView.layer.contents = topImage?.cgImage
...
//        self.playOrPauseBtn.setImage(QKUtils.imageName(name: "new_allPause_44x44_"), for: .selected)
    ...
    func makeSubviewsAction() {
        ...
        self.backBtn.addTarget(self, action: #selector(backClickAction), for: .touchUpInside)
    ...
    // MARK: - 返回
    @objc func backClickAction(btn: UIButton) {
        self.player?.enterPortrait(fullScreen: false, animated: true)
...
extension QKCustomLandScapeControlView: QKSliderViewChangeStateDelegate {
