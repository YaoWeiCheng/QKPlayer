...
class QKPlayerControlView: UIView {

    // 非全屏view
    private(set) var portraitControlView = QKCustomPortraitControlView()
    // 全屏view
    private(set) var landScapeControlView = QKCustomLandScapeControlView()
    // 加载loading
    ...
    public var isSeekToPlay = true {
        didSet {
            ...
            self.landScapeControlView.isSeekToPlay = oldValue
        }
    }
    // 控制层显示或者隐藏的回调
...
//        let min_view_h = self.frame.height
    ...
    func setupUI() {
        ...
        portraitControlView.sliderValueChanging = { [weak self](value, isForward) in
            let timeSecond = CGFloat(self?.player?.totalTime ?? 0) * value
            let draggedTime = QKUtils.converTimeSecond(timeSecond: Int(timeSecond))
            self?.landScapeControlView.sliderValueChanged(value: value, curretTimeStr: draggedTime)
            self?.cancelAutoFadeOutControlView()
        }
        portraitControlView.sliderValueChanged = { [weak self](value) in
            self?.landScapeControlView.sliderChangedEnded()
            self?.autoFadeOutControlView()
        }
        
        self.addSubview(self.landScapeControlView)
        landScapeControlView.sliderValueChanging = { [weak self](value, isForward) in
            let timeSecond = CGFloat(self?.player?.totalTime ?? 0) * value
            let draggedTime = QKUtils.converTimeSecond(timeSecond: Int(timeSecond))
            self?.portraitControlView.sliderValueChanged(value: value, curretTimeStr: draggedTime)
            self?.cancelAutoFadeOutControlView()
        }
        landScapeControlView.sliderValueChanged = { [weak self](value) in
            self?.portraitControlView.sliderChangedEnded()
            self?.autoFadeOutControlView()
        }
    ...
    func hiddenControlView(animate: Bool) {
        ...
        UIView.animate(withDuration: animate ? self.autoFadeTimeInterval : 0) {
            if self.player?.isFullScreen == true {
                self.landScapeControlView.hiddenControlView()
            } else {
    ...
    func showFullScreenModel(fullScreenModel: QKFullScreenMode) {
        ...
        self.portraitControlView.showFullScreenModel(fullScreenModel: fullScreenModel)
        self.landScapeControlView.showFullScreenModel(fullScreenModel: fullScreenModel)
    ...
    func resetControlView() {
        ...
        self.landScapeControlView.isHidden = !(self.player?.isFullScreen == true)
        self.failBtn.isHidden = true
...
extension QKPlayerControlView: QKPlayerMediaControl {
    ...
    var player: QKPlayerController? {
        ...
        set {
            ...
            self.landScapeControlView.player = newValue
        }
    }
...
//        self.hiddenControlView(animate: true)
    ...
    func videoPlayer(playerControl: QKPlayerController, loadStateChange state: QKPlayerLoadState) {
        if state == .prepare {
            ...
            self.landScapeControlView.playBtnSelectedState(selected: playerControl.currentPlayerManager?.shouldAutoPlay ?? false)
        }
    ...
    func videoPlayer(playerControl: QKPlayerController, currentTime: TimeInterval, totalTime: TimeInterval) {
        self.landScapeControlView.videoPlayer(playerControl: playerControl, currentTime: currentTime, totalTime: totalTime)
    ...
    func videoPlayer(playerControl: QKPlayerController, bufferTime: TimeInterval) {
        self.landScapeControlView.videoPlayer(playerControl: playerControl, bufferTime: bufferTime)
...
//    }
    ...
    func videoPlayer(playerControl: QKPlayerController, orientationWillChange observer: QKOrientationObserver) {
        self.portraitControlView.isHidden = observer.isFullScreen
        self.landScapeControlView.isHidden = !observer.isFullScreen
        if self.isControlViewAppeared == true {
            self.showControlView(animate: false)
        } else {
            self.hiddenControlView(animate: false)
        }
